...
description: Create viewer links for MNWHILE FlowKit diagrams and embed them into GitHub-flavored documentation workflows.
---

MNWHILE FlowKit diagrams can be embedded in any GitHub README or Markdown file as interactive, read-only views. No server setup or GitHub App required.
...
```
https://app.mnwhile-flowkit.com/#/view?flow=~COMPRESSED_DSL
```
...
The easiest path is to use the MNWHILE FlowKit MCP server's `create_viewer_url` tool or the editor's **Share / Embed** action. Both create a compressed, URL-safe viewer link that opens on the app domain:

```text
https://app.mnwhile-flowkit.com/#/view?flow=~...
```

### 3. Embed in your README

```markdown
[![Architecture Diagram](https://mnwhile-flowkit.com/og-diagram.png)](https://app.mnwhile-flowkit.com/#/view?flow=PASTE_ENCODED_VALUE_HERE)
```
...
```markdown
[View Architecture Diagram →](https://app.mnwhile-flowkit.com/#/view?flow=PASTE_ENCODED_VALUE_HERE)
```
...
[View Architecture →](https://app.mnwhile-flowkit.com/#/view?flow=...)
```

## Encoding helper

You can also export the viewer URL directly from the MNWHILE FlowKit editor:
