...
import { createYjsWebRtcCollaborationTransport } from './yjsWebrtcTransport';
...
describe('yjs realtime collaboration transport', () => {
  it('emits remote operations and ignores local operations', () => {
    ...
    const transport = createYjsWebRtcCollaborationTransport({
    ...
    transport.connect({ roomId: 'room-1', clientId: 'client-a', signalingServers: [] }, listener);
  ...
  it('emits remote presence updates and ignores local presence updates', () => {
    const awareness = new FakeAwareness();
    const transport = createYjsWebRtcCollaborationTransport({
      createProvider: () => ({
        awareness,
    ...
    expect(listener).toHaveBeenCalledTimes(1);
    expect(listener.mock.calls[0][0].type).toBe('presence');
    expect(listener.mock.calls[0][0].fromClientId).toBe('client-b');
  ...
  it('syncs operations between two realtime transports in the same room', () => {
    ...
    const transportA = createYjsWebRtcCollaborationTransport({
    ...
    const transportB = createYjsWebRtcCollaborationTransport({
      createProvider: (roomId, doc) => {
        const awareness = new FakeAwareness();
        const disconnect = hub.connect(roomId, doc);
        return { awareness, destroy: disconnect };
    ...
    transportA.connect({ roomId: 'room-1', clientId: 'client-a', signalingServers: [] }, listenerA);
    transportB.connect({ roomId: 'room-1', clientId: 'client-b', signalingServers: [] }, listenerB);
