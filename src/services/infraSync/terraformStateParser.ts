...
export function parseTerraformState(input: string): InfraSyncResult {
    ...
    const filtered = validResources.filter((r) => !SKIP_TYPES.has(r.type) && !r.type.startsWith('aws_iam_'));
    ...
    for (const resource of filtered) {
        ...
        const attrs = firstInstance && isTfStateResourceInstance(firstInstance) ? firstInstance.attributes : undefined;

        const tagName = attrs?.tags?.['Name'];
        const attrName = typeof attrs?.name === 'string' ? attrs.name : undefined;
        const attrFunctionName = typeof attrs?.function_name === 'string' ? attrs.function_name : undefined;
        const attrDbIdentifier = typeof attrs?.db_instance_identifier === 'string' ? attrs.db_instance_identifier : undefined;
