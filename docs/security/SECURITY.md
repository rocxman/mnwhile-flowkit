...
MNWHILE FlowKit is currently a pre-1.0 project. Security fixes are applied on a best-effort basis to the latest active code line only.
...
MNWHILE FlowKit is a browser-first, local-first application. Relevant security areas include:
...
MNWHILE FlowKit is **fully local-first**. No diagram data, API keys, or user content is sent to MNWHILE FlowKit servers. There are no MNWHILE FlowKit servers.
...
MNWHILE FlowKit uses a Bring-Your-Own-Key (BYOK) model:

- API keys are entered in **Settings → AI** and stored in **localStorage** under a dedicated namespace.
- Keys are sent **directly from your browser to the AI provider** (OpenAI, Anthropic, Google, etc.) — not proxied through any MNWHILE FlowKit service.
